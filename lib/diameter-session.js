...
function DiameterSession(options, socket) {
    if (!(this instanceof DiameterSession)) {
        return new DiameterSession(options, socket);
    }
    options = options || {};
    var self = this;
    self.socket = socket;
    ...
    self.createRequest = function(application, command) {
        if (self.sessionId === undefined) {
            self.sessionId = diameterUtil.random32BitNumber();
        }
        return diameterCodec.constructRequest(application, command, self.sessionId);
    };
